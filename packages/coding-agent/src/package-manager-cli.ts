...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|shibui] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Short forms:
  ...
  ${APP_NAME} update shibui         Update ${APP_NAME} only (self works as alias)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === APP_NAME || source === "shibui" || source === "pi";
			if (sourceIsSelf) {
