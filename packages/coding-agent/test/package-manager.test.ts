...
import { CONFIG_DIR_NAME, ENV_OFFLINE } from "../src/config.js";
...
Content`,
		...
		it("should resolve project paths relative to project config", async () => {
...
Content`,
			...
			expect(sharedSkillMatches[0]?.metadata.scope).toBe("project");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should dedupe user skill entries when the user config skills dir is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to project settings base", () => {
